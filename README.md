...
| モード | 向いている用途 |
| --- | --- |
| `auto` | おまかせ。情報を削らず、AI らしい均質な文章リズムを崩して人が書いたような日本語にする。**既定値** |
| `light` | 誤字・助詞・読点など最低限だけ直したい |
| `natural` | 自然で読みやすい日本語にしたい |
| `business` | メール、告知、提案文など落ち着いた対外文 |
| `social` | SNS、Slack、社内チャットなど温度感を残した文章 |

たとえば「全体を変えすぎず、最低限だけ読みやすく」は `light` が向いています。

読み手が決まっているときは `--audience "教育委員会の担当者"` のように渡すと、その読者が一読で分かる語彙と文の長さに合わせます。

## AI 臭の簡易 lint

report.json の `readability` に、原稿と出力それぞれのリズム指標を並べます。文長のばらつき（CV）、burstiness、隣接する文の長さの相関、語尾トップ 2 の占有率、同じ語尾の連続数、段落ごとの文数のばらつき、体言止め率、「〜ではなく」率、読点数、定型表現の種類です。[AI 臭の正体は語彙より文章リズムの均質さ](https://zenn.dev/coji/articles/natural-japanese-ai-smell-lint)という実測に基づく指標で、閾値は [coji/natural-japanese](https://github.com/coji/natural-japanese) の人間コーパス検証値を参考にしています（この Skill 自身では未検証）。

検査はすべて **原稿と出力の差分** で行います。「AI が書いたか」を判定するのではなく、「Gemini が AI 臭を持ち込んだか」を見ます。

- 出力の文長が原文より均質になった、語尾が単調になった
- 太字・番号付き箇条書き・見出し・コロンで終わる行が増えた
- 「結論から申し上げますと」「以下の通りです」「〜を浮き彫りにしており」のような、リライト側モデルの定型表現が増えた（単発は info、同一段落に複数なら warn）
- 原文に無い評価語（「安全に」「厳重に」）やカタカナ語が増えた

指摘は `findings` に `info / warn / critical` のレベル付きで入り、`warnings` は warn 以上の文面です。原稿が 100 字未満なら指標なし、400 字未満なら全文リズム指標なしです。原稿側の診断は Gemini にもヒントとして渡し、直すかどうかは文脈で判断させます。数値は判断材料であり合否ではありません。
...
└─ 使用モデル、トークン情報、AI 臭 lint、数値変更・評価語追加・カタカナ語追加などの警告
```
