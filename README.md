...
report.json の `readability` に、原稿と出力それぞれの「文長のばらつき（CV）」「段落ごとの文数のばらつき」「体言止め率」「『〜ではなく』率」を並べます。[AI 臭の正体は語彙より文章リズムの均質さ](https://zenn.dev/coji/articles/natural-japanese-ai-smell-lint)という実測に基づく指標で、出力が原文より均質になっていれば警告します。原稿側の診断は Gemini にもヒントとして渡し、直すかどうかは文脈で判断させます。数値は判断材料であり合否ではありません。
