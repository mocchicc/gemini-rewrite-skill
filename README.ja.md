...
| `natural` | 自然で読みやすい日本語。既定 |
| `business` | 明瞭で落ち着いた対外文。過剰な敬語を増やさない |
| `social` | SNS・社内チャット向け。熱量を保ち、煽りは加えない |

SNS用でも、明示がなければ文字数制限や要約は行いません。編集ルールは `gemini-rewrite/references/rewrite-system.md` にあります。普段の自分の文章や会社のスタイルガイドは `--style-file` で今回の参考として渡せます。参考資料の事実を原稿へ混ぜないよう指示しています。
