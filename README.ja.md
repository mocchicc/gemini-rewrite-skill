...
`readability` には原稿と出力の簡易lint（文数、平均文長、文長の変動係数、段落ごとの文数の変動係数、体言止め率、「ではなく」率、抽象名詞数）を並べます。出力の文長が原文より明らかに均質になった場合は警告します。閾値は人間コーパスで検証したものではなく参考値です。原稿側の疑いは `source_diagnostics_hints` としてGeminiにも渡します。主張の強さが変わる典型例なので、対外文書では原文に戻すか不採用にしてください。文字数が原文の70%未満または140%超の場合も警告します。この閾値は便利な注意喚起であり、品質評価の基準や意味保持の保証ではありません。
