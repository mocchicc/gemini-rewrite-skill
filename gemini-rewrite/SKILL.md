...
5. 元ファイルとは別の、未使用の出力先を指定して実行する。既定は`--mode natural --thinking low`。構成変更を伴う編集だけ`--thinking medium`を明示する。最小修正は`light`、対外文は`business`、SNS・社内チャットは`social`。モデル変更や再試行の増加はユーザーが指定した場合だけ行う。実行時に表示される「モデル: … (設定元: …)」を確認し、ユーザーの指定と異なる設定元なら止めて報告する。
...
7. 原文と差分を比較し、固有名詞、数値、日付、条件・例外、否定、留保、責任主体、情報欠落を確認する。レポートの`evaluative_terms_added`（原文に無い「安全に」「厳重に」「にとどまる」等）は主張の強さが変わる典型例なので、対外文書では原文に戻すか不合格にする。数値・文字数警告は参考であり、警告がないことは意味の保持を保証しない。
