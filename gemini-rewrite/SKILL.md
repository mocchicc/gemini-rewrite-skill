...
7. 原文と差分を比較し、固有名詞、数値、日付、条件・例外、否定、留保、責任主体、情報欠落を確認する。レポートの`findings`は`level`（info / warn / critical）付きの指摘で、`warnings`はそのうち warn 以上の文面。`readability`は原稿と出力の簡易lint（文長のばらつき・burstiness・隣接文長の相関・語尾の単調さ・段落の文数・体言止め・「ではなく」率・定型表現の種類）で、出力が原文より均質になった、語尾が単調になった、太字や箇条書きや定型表現が増えた、といった「Geminiが持ち込んだAI臭」を差分で拾う。数値は判断材料であり合否ではない。原稿が100字未満なら指標なし、400字未満なら全文リズム指標なし。立場・主体・具体性の欠落は機械でもGeminiでも直せないので、呼び出し元が原文と照らして判断し、Geminiに補わせない。レポートの`evaluative_terms_added`（原文に無い「安全に」「厳重に」「にとどまる」等）は主張の強さが変わる典型例なので、対外文書では原文に戻すか不合格にする。数値・文字数警告は参考であり、警告がないことは意味の保持を保証しない。
